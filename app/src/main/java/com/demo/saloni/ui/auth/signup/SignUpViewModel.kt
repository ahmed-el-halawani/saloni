...
class SignUpViewModel : ViewModel() {
    ...
    fun signUpSalon(
        image: Uri?, name: String, phoneNumber: String, email: String, password: String, facebook: String, instagram: String, twitter: String
    ) = flow {
        emit(State.Loading())
        try {
            val profile = authService.signUpSalon(image, name, phoneNumber, email, password, facebook, instagram, twitter)
            emit(State.Success(profile));
        } catch (e: Throwable) {
            emit(State.Error(e.message ?: e.localizedMessage))
        }
